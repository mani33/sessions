%{
stim.MovieInfo (computed) # my newest table
->stim.StimTrialGroup
-----
...
classdef MovieInfo < dj.Relvar & dj.AutoPopulate
	...
	properties(Constant)
		table = dj.Table('stim.MovieInfo')
        popRel = stim.StimTrialGroup & acq.Stimulation('exp_type="MoviesExperiment"');
	end
	...
	methods(Access=protected)
		...
		function makeTuples(self, key)
            ...
            tuple = key;
            stimInfo = fetch(stim.StimTrialGroup(key), '*');
            conditions = fetch(stim.StimConditions & key,'*');
            trials = fetch(stim.StimTrials * stim.StimTrialEvents ...
